...
class GlobalExceptionHandler {
    ...
    fun handleRedisConnectionException(e: RedisConnectionFailureException, request: HttpServletRequest): ResponseEntity<ErrorResponse> {
        val traceId = getTraceId()
        val path = request.requestURI

        logger.error("Redis 연결 오류 발생, traceId: {}", traceId, e)
        return createErrorResponse(ErrorCode.REDIS_CONNECTION_FAILED, e.message, traceId, path)
    }
    ...
    fun handleConstraintViolationException(
        ...
        logger.warn("제약 조건 위반: $errorMessages, traceId: $traceId")
        return createErrorResponse(ErrorCode.INVALID_REQUEST, errorMessages, traceId, path)
    }
    ...
    fun handleMethodNotSupportedException(
        ...
        val message = "지원하지 않는 HTTP 메소드입니다. 요청: ${ex.method}, 지원 메소드: $supportedMethods"
        
        logger.warn("지원하지 않는 HTTP 메소드: $message, traceId: $traceId")
        return createErrorResponse(ErrorCode.INVALID_REQUEST, message, traceId, path)
    }
    ...
    fun handleNoHandlerFoundException(
        ...
        val traceId = getTraceId()
        val path = request.requestURI
        val message = "요청한 리소스를 찾을 수 없습니다: ${ex.httpMethod} ${ex.requestURL}"
        
        logger.warn("리소스를 찾을 수 없음: $message, traceId: $traceId")
        return createErrorResponse(ErrorCode.ENTITY_NOT_FOUND, message, traceId, path)
    }
